...
interface RecentCaseStudiesTableProps {
  caseStudies: CaseStudy[];
  limit?: number;
}

export function RecentCaseStudiesTable({
  caseStudies,
  limit = 5,
}: RecentCaseStudiesTableProps) {
  const rows = caseStudies.slice(0, limit);
      ...
      <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4 dark:border-zinc-800 sm:px-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">
            Your latest published and draft case studies.
          </p>
        </div>
        <Link
          href="/case-studies"
